...
export default function ChapterPage({
  ...
  // Step 2 unlocks after user has interacted with game outcome (OP > 0 or result changed)
  const step1Complete = objectivePoints > 0;
  // Step 3 (submit) unlocks after promotion roll is entered
  ...
  return (
    <div style={{ animation: "slide-up 0.5s ease-out" }} className="pb-12">
            ...
            <div className="p-3 bg-surface/30 border border-cyan-dim/20">
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  {matchInfo.yourTeam.faction1 && <span className="font-[family-name:var(--font-mono)] text-[10px] text-text-muted">{matchInfo.yourTeam.faction1}</span>}
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-1.5 h-1.5 bg-cyan rounded-full shrink-0" />
                  <span className="font-[family-name:var(--font-mono)] text-xs text-text-primary">{matchInfo.yourTeam.player2}</span>
                  {matchInfo.yourTeam.faction2 && <span className="font-[family-name:var(--font-mono)] text-[10px] text-text-muted">{matchInfo.yourTeam.faction2}</span>}
                </div>
              </div>
            </div>
            ...
            <div className="p-3 bg-surface/30 border border-red-dim/20">
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  {matchInfo.opponentTeam.faction1 && <span className="font-[family-name:var(--font-mono)] text-[10px] text-text-muted">{matchInfo.opponentTeam.faction1}</span>}
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-1.5 h-1.5 bg-red rounded-full shrink-0" />
                  <span className="font-[family-name:var(--font-mono)] text-xs text-text-primary">{matchInfo.opponentTeam.player2}</span>
                  {matchInfo.opponentTeam.faction2 && <span className="font-[family-name:var(--font-mono)] text-[10px] text-text-muted">{matchInfo.opponentTeam.faction2}</span>}
                </div>
              </div>
            </div>
          </div>
      ...
      {/* ═══════════════════════════════════════════════════════════ */}
      {/* PHASE 1: AFTER-ACTION REPORT (Steps 1 → 2 → 3: Submit)    */}
      {/* ═══════════════════════════════════════════════════════════ */}
            ...
            <div key={step.num} className="flex items-center gap-2 flex-1">
              <div className={`flex items-center gap-2 px-3 py-2 border transition-all flex-1 ${
                ...
                <span className={`font-[family-name:var(--font-orbitron)] text-xs ${
                  step.done ? "text-cyan" : step.active ? "text-amber" : "text-text-muted"
                }`}>{step.num}</span>
                <span className={`font-[family-name:var(--font-mono)] text-[10px] tracking-wider ${
                  step.done ? "text-cyan" : step.active ? "text-text-primary" : "text-text-muted"
                }`}>{step.label}</span>
                {step.done && (
                  <svg className="w-3 h-3 text-cyan ml-auto shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
      ...
      <div className={`panel panel-glow p-4 sm:p-5 mb-6 border-cyan-dim/40 ${hasSubmitted ? "opacity-60 pointer-events-none" : ""}`}>
          ...
          {resultOptions.map((opt) => (
            <button
              ...
              className={`py-2.5 sm:py-3 border font-[family-name:var(--font-orbitron)] text-xs sm:text-sm tracking-wider transition-all cursor-pointer ${
                result === opt.value ? opt.color : "border-border text-text-muted hover:border-border-bright hover:text-text-secondary"
              }`}
            >
              {opt.label}
            </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div>
            <label className="data-label font-[family-name:var(--font-mono)] block mb-2">YOUR ARMY SURVIVED (%)</label>
            <div className="flex items-center gap-3">
              <input type="range" min="0" max="100" step="5" value={armySurvived} onChange={(e) => setArmySurvived(Number(e.target.value))} className="flex-1 accent-cyan" />
              <span className="font-[family-name:var(--font-mono)] text-sm text-text-primary w-12 text-right">{armySurvived}%</span>
            </div>
            <div className="flex justify-between mt-1">
              <span className="font-[family-name:var(--font-mono)] text-xs text-red">&lt;25% penalty</span>
              <span className="font-[family-name:var(--font-mono)] text-xs text-green">&gt;75% bonus</span>
            </div>
          </div>
          <div>
            <label className="data-label font-[family-name:var(--font-mono)] block mb-2">ENEMY SURVIVED (%)</label>
            <div className="flex items-center gap-3">
              <input type="range" min="0" max="100" step="5" value={enemySurvived} onChange={(e) => setEnemySurvived(Number(e.target.value))} className="flex-1 accent-amber" />
              <span className="font-[family-name:var(--font-mono)] text-sm text-text-primary w-12 text-right">{enemySurvived}%</span>
            </div>
            <div className="flex justify-between mt-1">
              <span className="font-[family-name:var(--font-mono)] text-xs text-green">&le;25% bonus</span>
              <span className="font-[family-name:var(--font-mono)] text-xs text-text-muted">&nbsp;</span>
            </div>
          </div>
        </div>

        {/* Inline live calculations */}
        <div className="mt-6 pt-4 border-t border-border/50">
          <div className="grid grid-cols-3 gap-4">
            <div className="text-center p-3 bg-surface/30 border border-border/20">
              ...
              <div className="font-[family-name:var(--font-mono)] text-[10px] text-text-secondary mt-1">{objectivePoints} OP{result === "win" ? " + 1 WIN" : ""}</div>
            </div>
            <div className="text-center p-3 bg-surface/30 border border-border/20">
      ...
      {/* ── STEP 2: PROMOTION ROLL (unlocks after Step 1) ─────── */}
      <div className={`panel p-4 sm:p-5 mb-6 transition-all duration-300 ${
        ...
        {!step1Complete && !hasSubmitted && (
          <div className="text-center py-6">
            <p className="font-[family-name:var(--font-mono)] text-xs text-text-muted">Complete Step 1 to unlock promotion roll.</p>
          </div>
        )}
          ...
          <>
              ...
              <div className="space-y-2">
                ...
                {Object.entries(promotionCalc.modifiers).map(([key, value]) => (
                  <div key={key} className="flex items-center justify-between px-3 py-1.5 bg-surface/50 border border-border/30">
                    ...
                    <span className={`font-[family-name:var(--font-mono)] text-xs font-bold ${value >= 0 ? "text-green" : "text-red"}`}>
                      {value >= 0 ? "+" : ""}{value}
                    </span>
                  </div>
                ))}
            ...
            <div className="flex flex-wrap items-center gap-3 sm:gap-4">
              ...
              <input
                ...
                value={promotionRoll ?? ""}
                onChange={(e) => setPromotionRoll(e.target.value ? Number(e.target.value) : null)}
                placeholder="1-20"
      ...
      {/* ── STEP 3: REVIEW & SUBMIT (unlocks after Step 2) ────── */}
      {!hasSubmitted && (
        <div className={`panel panel-glow p-4 sm:p-5 mb-8 transition-all duration-300 ${
          !step2Complete ? "opacity-30 pointer-events-none border-border/30" : "border-cyan-dim/40"
        }`}>
          <div className="flex items-center gap-3 mb-4 pb-3 border-b border-border">
          ...
          {!step2Complete && (
            <div className="text-center py-6">
              <p className="font-[family-name:var(--font-mono)] text-xs text-text-muted">Complete Steps 1 & 2 to submit your report.</p>
            </div>
          )}
            ...
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
                <div className="px-3 py-2 bg-surface/30 border border-border/20">
                  ...
                  <div className={`font-[family-name:var(--font-orbitron)] text-sm uppercase ${result === "win" ? "text-green" : result === "draw" ? "text-amber" : "text-red"}`}>{result}</div>
                </div>
                <div className="px-3 py-2 bg-surface/30 border border-border/20">
                ...
                <div className="px-3 py-2 bg-surface/30 border border-border/20">
                  ...
                  <div className={`font-[family-name:var(--font-orbitron)] text-sm ${promotionSuccess ? "text-green" : "text-red"}`}>
                    {promotionSuccess ? "PROMOTED" : "FAILED"}
                  </div>
                </div>
              </div>
              ...
              <div className="p-3 bg-amber/5 border border-amber-dim/30 mb-5">
                <p className="font-[family-name:var(--font-mono)] text-xs text-amber">
                  Once submitted, your game result will be locked and cannot be edited.
                </p>
              </div>
      ...
      {/* ═══════════════════════════════════════════════════════════ */}
      {/* PHASE 2: SPEND XP (unlocks after submit)                   */}
      {/* ═══════════════════════════════════════════════════════════ */}
      ...
      {hasSubmitted && (
        <div className="flex items-center gap-4 my-8">
          ...
          <span className="font-[family-name:var(--font-orbitron)] text-xs tracking-[0.2em] text-cyan">SPEND YOUR XP</span>
          <div className="flex-1 h-px bg-gradient-to-r from-transparent via-cyan-dim/40 to-transparent" />
        </div>
      )}

      {/* XP spending section — visible but locked before submit */}
      <div className={`transition-all duration-300 ${!hasSubmitted ? "opacity-25 pointer-events-none" : ""}`}>

        {/* Locked overlay message when not submitted */}
        {!hasSubmitted && (
          <div className="mb-6 px-4 py-3 bg-surface/30 border border-border/30 text-center">
            <p className="font-[family-name:var(--font-mono)] text-xs text-text-muted">
              Submit your After-Action Report to unlock XP spending.
            </p>
          </div>
        )}
            ...
            <div className="grid grid-cols-6 gap-1.5 sm:gap-2 min-w-[600px]">
              ...
              {[1, 2, 3, 4].map((level) =>
                CEB_COLUMNS.map((col) => {
                  ...
                  const previousOwned = level === 1 || purchasedCEB[col.key]?.includes(level - 1);
                  const canPurchase = !isPurchased && previousOwned && xpSummary.available >= CEB_LEVEL_COSTS[level];
                  ...
                  return (
                    <button
                      ...
                      onClick={() => canPurchase && handlePurchaseCEB(col.key, level)}
                      className={`relative p-2 sm:p-2.5 border text-left transition-all duration-200 min-h-[60px] sm:min-h-[72px] cursor-pointer ${
                        isPurchased ? "bg-cyan/15 border-cyan-dim/40 hover:border-cyan"
                          : canPurchase ? "bg-surface/50 border-border hover:border-amber-dim hover:bg-amber/5"
                          : "bg-surface/20 border-border/30 opacity-40 cursor-not-allowed"
                      }`}
                    >
                      <div className="flex items-center justify-between mb-1">
                        <span className={`font-[family-name:var(--font-mono)] text-xs font-bold ${isPurchased ? "text-cyan" : "text-text-muted"}`}>L{level}</span>
                        <span className={`font-[family-name:var(--font-mono)] text-xs ${isPurchased ? "text-cyan-dim" : "text-text-muted"}`}>{CEB_LEVEL_COSTS[level]} XP</span>
                      </div>
            ...
            {(() => {
              ...
              return (
                <button
                  ...
                  onClick={() => { if (canPurchaseL5) { const c = CEB_COLUMNS.find((col) => purchasedCEB[col.key]?.includes(4)); if (c) handlePurchaseCEB(c.key, 5); } }}
        ...
        <div className="panel p-4 sm:p-5 mb-8">
          <div className="flex items-center justify-between mb-4 pb-3 border-b border-border">
            <div className="flex items-center gap-3">
              ...
              {specOps.length > 0 && (
                <span className="font-[family-name:var(--font-mono)] text-xs text-text-secondary">{specOps[0].total_xp_spent} XP INVESTED</span>
              )}
            </div>
            {!showAddSkill && (
              <button
            ...
            {CONSUMABLE_DEFS.map((consumable) => {
              ...
              return (
                <button
                  ...
                  onClick={() => !isPurchased && canAfford && handlePurchaseConsumable(consumable.key, consumable.cost)}
                  className={`p-3 sm:p-4 border text-left transition-all duration-200 cursor-pointer ${
                    isPurchased ? "bg-red/10 border-red-dim/40"
                      : canAfford ? "bg-surface/30 border-border/50 hover:border-amber-dim hover:bg-amber/5"
                      : "bg-surface/20 border-border/30 opacity-40 cursor-not-allowed"
                  }`}
                >
